...
const natakhtariCopy: Record<Locale, BlogSeoPostContent> = {
  ...
  ru: article(
    ...
    'Что знать об аэропорте Натахтари рядом с Тбилиси: какие рейсы Vanilla Sky его используют, путаница с Ново-Алексеевкой, трансфер и планирование.',
    ...
    [
      ...
      section('Ново-Алексеевка — это не аэропорт Натахтари', [
        'Ново-Алексеевка — более старое название, которое иногда связывают с Международным аэропортом Тбилиси, а не с аэропортом Натахтари, который Vanilla Sky использует для внутренних рейсов из тбилисского района.',
        'Если в результатах поиска встречается Ново-Алексеевка, Международный аэропорт Тбилиси или код TBS, перепроверьте маршрут: страницы внутренних рейсов Vanilla Sky обычно указывают именно на Натахтари. Актуальное название аэропорта уточняйте на официальном сайте бронирования.'
      ]),
